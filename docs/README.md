...
| [ghostty-setup.md](ghostty-setup.md) | Ghostty | 터미널 설치 + `ghostty/config` 심링크 (macOS 전용) |
...
| [claude-setup.md](claude-setup.md) | Claude Code | 슬래시 커맨드(`~/.claude/commands/`) 파일별 심링크 |
...
6. ghostty-setup # 터미널 설정 심링크 (macOS 전용)
7. git-setup    # git delta / 서명 / lfs
8. yazi-setup   # 파일 매니저 (+ EDITOR=nvim 연동)
9. claude-setup # Claude Code 슬래시 커맨드 심링크 (독립적, 순서 무관)
```
